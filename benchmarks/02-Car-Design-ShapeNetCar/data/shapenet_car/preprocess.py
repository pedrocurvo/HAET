...
# pip install open3d meshio torch tempfile
import os
import tempfile
...
def safe_read_mesh(uri):
    try:
        mesh = meshio.read(uri)
        if not mesh.cells or mesh.cells[0].type != "quad":
            raise ValueError("Invalid cell type or empty mesh")
        return mesh
    except Exception as e:
        print(f"[Warning] Failed to read mesh: {uri}: {e}")
        return None


def sdf(mesh, resolution):
    try:
        quads = mesh.cells_dict.get("quad", None)
        if quads is None:
            raise ValueError("No quad cells in mesh")
        ...
        if not open3d_mesh.has_triangles() or not open3d_mesh.has_vertices():
            raise ValueError("Mesh has no valid triangles/vertices")
        ...
        print(f"[Warning] Failed to compute SDF: {e}")
        return None
...
def main(src, dst):
    ...
    for uri in tqdm(uris):
        ...
        vtk_path = uri / "quadpress_smpl.vtk"
        press_path = uri / "press.npy"
        mesh = safe_read_mesh(vtk_path)
        if mesh is None:
            continue
        ...
        try:
            ...
            pressure = torch.from_numpy(np.load(press_path)[unique]).float()
            torch.save(mesh_points, out / "mesh_points.th")
            torch.save(pressure, out / "pressure.th")
        except Exception as e:
            print(f"[Warning] Failed to extract and save data for {uri}: {e}")
            continue

        for resolution in [32, 40, 48, 64, 80]:
            sdf_tensor = sdf(mesh, resolution=resolution)
            if sdf_tensor is not None:
                torch.save(sdf_tensor, out / f"sdf_res{resolution}.th")

...
class ErwinTransolver(nn.Module):
    ...
    def __init__(
        ...
        self.memory_states = nn.Parameter(torch.randn(1, 1, self.memory_tokens, dim_head))
        init_pos = self.uniform_memory_positions(1, self.memory_tokens, dimensionality).clone()
    ...
    def forward(self, x: torch.Tensor, pos: torch.Tensor = None) -> torch.Tensor:
        ...
        # Project input
        x_proj = self.in_project_x(x).view(B, N, HEADS, self.dim_head).transpose(1, 2)  # [B, H, N, D]
        pos_proj = pos.view(B, N, 1, self.dimensionality).expand(B, N, HEADS, self.dimensionality).transpose(1, 2)  # [B, H, N, D]
        ...
        out = out.transpose(1, 2).reshape(B, N, HEADS * self.dim_head)  # [B, N, C]
...
class Transolver_block(nn.Module):
    ...
    def __init__(
        ...
        self.Attn = ErwinTransolver(hidden_dim, heads=num_heads, dim_head=hidden_dim // num_heads,
                                     dropout=dropout, slice_num=slice_num, radius=radius, dimensionality=2)

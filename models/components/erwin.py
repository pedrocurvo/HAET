...
class MPNN(nn.Module):
    ...
    def compute_edge_attr(self, pos, edge_index):
        # Ensure consistent dtypes for edge attribute computation
        edge_attr = pos[edge_index[0]] - pos[edge_index[1]]
        return edge_attr.to(dtype=pos.dtype)

    def forward(self, x: torch.Tensor, pos: torch.Tensor, edge_index: torch.Tensor):
        edge_attr = self.compute_edge_attr(pos, edge_index)
        # Ensure edge_attr has the same dtype as x
...
class ErwinEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, pos: torch.Tensor, edge_index: torch.Tensor):
        x = self.embed_fn(x)
        if self.mp_steps > 0:
            # Ensure pos has the same dtype as x for MPNN processing
...
class BallPooling(nn.Module):
    ...
    def forward(self, node: Node) -> Node:
        ...
        with torch.no_grad():
            ...
            # Ensure rel_pos has the same dtype as node.x
            rel_pos = rel_pos.to(dtype=node.x.dtype)
...
class BallMSA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, pos: torch.Tensor):
        # Ensure pos has the same dtype as x for consistent processing
        pos = pos.to(dtype=x.dtype)
        rel_pos = self.compute_rel_pos(pos).to(dtype=x.dtype)
        x = x + self.pe_proj(rel_pos)
        q, k, v = rearrange(self.qkv(x), "(n m) (H E K) -> K n H m E", H=self.num_heads, m=self.ball_size, K=3)
        attn_mask = self.sigma_att * self.create_attention_mask(pos)
...
class ErwinTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Ensure consistent dtypes for Node creation
        node_positions_indexed = node_positions[tree_idx].to(dtype=x.dtype)
        
        node = Node(
            x=x[tree_idx],
            pos=node_positions_indexed,
